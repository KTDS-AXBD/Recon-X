...
| TD-34 | Decode-X svc-skill production `FOUNDRY_X_SECRET` + Foundry-X production `DECODE_X_HANDOFF_SECRET` | **Sprint 219 F355b 인증 secret 양측 미설정** — 세션 221 production 실증 선행조사에서 발견. (1) Decode-X production: `FOUNDRY_X_SECRET` secret 목록 미포함(INTERNAL_API_SECRET + OPENROUTER_API_KEY만), F355a "placeholder 등록" 주장 허위. (2) Foundry-X production: `internal-prototype-jobs.ts:21` expected=`DECODE_X_HANDOFF_SECRET`이지만 production secrets에 `DECODE_X_INTERNAL_SECRET`만 설정(이름 drift). 해결안: (a) shared secret 실값 결정(user), (b) Decode-X production `FOUNDRY_X_SECRET` put, (c) Foundry-X production `DECODE_X_HANDOFF_SECRET` put 또는 코드 네이밍을 `DECODE_X_INTERNAL_SECRET`로 정렬. **P0** | Production handoff 호출이 Foundry-X 측에서 401 `INVALID_INTERNAL_SECRET` 반환 — Sprint 219 F355b 코드는 production에서 한 번도 성공한 적 없음. Sprint 215 패턴 반복 | 2026-04-21 |
