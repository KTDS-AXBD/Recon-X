...
| TD-40 | `apps/app-web/e2e/` — auth.setup.ts 외 10개 spec 전체 skip | **F389 DEMO_USERS 폐기로 E2E auth 기반 전면 무효화** — Sprint 223 F389 DEMO_USERS 폐기 후 `auth.setup.ts`가 "서민원" 데모 카드 클릭으로 로그인하던 로직 실패 (element not found). CF Access JWT는 CI 환경에서 발급 불가 → `auth.spec.ts`(DEMO 3건), `rbac.spec.ts`(loginAs() 전체), `functional/upload/admin/poc-spec/verify/extract/organization/deliver.spec.ts`(protected route 전체) 임시 `test.describe.skip`. 해결안: (a) CF Access JWT mock (msw or Playwright route-intercept로 `CF_Authorization` cookie 주입) + `/auth/me` stub, (b) 또는 E2E 전용 `?demo=1` 로그인 우회 endpoint 추가. **S224 착수, F374 Feature Flag 실 분기 활성화 이후 연계 구현**. P1 | CI E2E pass = 테스트 1건만 실행 (auth redirect /welcome 확인). 보호 품질 0% — 빠른 CF mock 복원 필요 (세션 223 F389 대응) | 2026-04-21 |
