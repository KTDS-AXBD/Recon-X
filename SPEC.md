...
| TD-45 | 26 test fail — svc-extraction(`llm.test.ts` 5 + `llm-matcher.test.ts` 6) + svc-skill(`classifier.test.ts` 9 + `evaluate.test.ts` 5 + `tacit-interview.test.ts` 1) fetch mock response body가 svc-llm-router `{success,data}` 포맷 잔존 | TD-44 Phase 1(세션 238)에서 llm-client.ts OpenRouter 포맷 교체 후 5 테스트 파일의 fetch mock 미전환. svc-policy `caller/consensus/semantic` 3 파일은 세션 238에서 재작성(144/144 PASS). 남은 5 파일 패턴 동일(helper `llmSuccess`/`makeLlmResponse` 함수에서 `{success:true, data:{content}}` → `{choices:[{message:{content}}]}` 교체). 각 파일 헬퍼 1~3곳 수정 예상 1h. CI Typecheck&Test job fail 원인. **P2**(core 기능 동작엔 영향 없음, 테스트 커버리지 회귀만 발생). 일괄 Edit 또는 Python 정규식 치환으로 처리 가능 | 별도 Sprint 또는 Master 단독 작업으로 처리 | 2026-04-23 |
