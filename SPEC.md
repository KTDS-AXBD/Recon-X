...
- [🔧] Sprint 214a (REQ-035 Phase 2 D1, 세션 217~): **Track A Fill — 예산 + 구매** — 각 소스 원장 기반 Empty Slot 발굴·Fill (각 ~2~3 슬롯). Phase 1 "충전" 방법론 재활용. 대상: `.decode-x/spec-containers/lpon-budget/`, `lpon-purchase/` (신규). **KPI**: 완결성 ≥95%, AI-Ready 6기준 ≥70%, 소스 출처 추적성 100%
- [🔧] Sprint 214b (REQ-035 Phase 2 D2, 세션 217~): **Track A Fill — 결제 + 환불** (Track B 선행 필수). 대상: `.decode-x/spec-containers/lpon-payment/`, `lpon-refund/`. **KPI**: 동일. 결제 Fill은 Sprint 215 E2E 입력이 되므로 **214a/214c보다 우선 merge**
- [🔧] Sprint 214c (REQ-035 Phase 2 D3, 세션 217~): **Track A Fill — 선물 + 정산**. 대상: `.decode-x/spec-containers/lpon-gift/`, `lpon-settlement/`. **KPI**: 동일
